...
export default function Dashboard({ auth }) {
    ...
    return (
        <AuthenticatedLayout user={auth.user}>
            ...
            <div className="flex pt-4 pl-4" style={{ height: '800px' }}>
                <aside className="bg-gray-500 w-40 flex flex-col items-center py-5 rounded-full">
                    <img src="/assets/logo.JPG" alt="Logo" className="w-35 h-35 mb-1 pt-5" />
                    <nav className="flex flex-col gap-10 mt-12">

                        <Link href="/dashboard" className="text-yellow-500 py-10">
                            <svg className="w-16 h-16" fill="currentColor" viewBox="0 0 576 512">
                            <path d="M575.8 255.5c0 18-15 32.1-32 32.1h-32l.7 160.2c0 2.7-.2 5.4-.5 8.1V472c0 22.1-17.9 40-40 40H456c-1.1 0-2.2 0-3.3-.1c-1.4 .1-2.8 .1-4.2 .1H416 392c-22.1 0-40-17.9-40-40V448 384c0-17.7-14.3-32-32-32H256c-17.7 0-32 14.3-32 32v64 24c0 22.1-17.9 40-40 40H160 128.1c-1.5 0-3-.1-4.5-.2c-1.2 .1-2.4 .2-3.6 .2H104c-22.1 0-40-17.9-40-40V360c0-.9 0-1.9 .1-2.8V287.6H32c-18 0-32-14-32-32.1c0-9 3-17 10-24L266.4 8c7-7 15-8 22-8s15 2 21 7L564.8 231.5c8 7 12 15 11 24z"/>
                            </svg>
                        </Link>


                        <Link href="/addcart" className="text-yellow-500 py-10">
                            <svg className="w-16 h-16" fill="currentColor" viewBox="0 0 576 512">
                            <path d="M0 24C0 10.7 10.7 0 24 0H69.5c22 0 41.5 12.8 50.6 32h411c26.3 0 45.5 25 38.6 50.4l-41 152.3c-8.5 31.4-37 53.3-69.5 53.3H170.7l5.4 28.5c2.2 11.3 12.1 19.5 23.6 19.5H488c13.3 0 24 10.7 24 24s-10.7 24-24 24H199.7c-34.6 0-64.3-24.6-70.7-58.5L77.4 54.5c-.7-3.8-4-6.5-7.9-6.5H24C10.7 48 0 37.3 0 24zM128 464a48 48 0 1 1 96 0 48 48 0 1 1 -96 0zm336-48a48 48 0 1 1 0 96 48 48 0 1 1 0-96z"/>
                            </svg>
                        </Link>

                        <Link href="/transaction" className="text-yellow-500 py-10">
                            <svg className="w-16 h-16" fill="currentColor" viewBox="0 0 400 512">
                                                ...
                                                {handleSearch().map(product => (
                                                    <div key={product.id} className="w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/5 p-2">
                                                        <div className="bg-white p-4 rounded-lg shadow-md">
                                                            <img src={`/storage/${product.image}`} alt={product.name} className="w-full h-32 object-contain rounded-lg mb-2" />   
                                                            ...
                                                            <p className="text-green-600 font-bold text-center mb-4">${product.price}</p>
                                                            <div className="flex justify-center mb-4">
                                                                <button 
                                                                    onClick={() => handleBuy(product.id)}
                                                                    className={`mt-2 px-4 py-2 rounded-lg ${product.stock > 0 ? 'bg-green-500' : 'bg-red-500'}`}
